...
void Coordinates::print (Dstr &text_out, Pad pad) const {
  if (_isNull) {
    ...
    char temp[80];
    if (pad == fixedWidth)
      sprintf (temp, "%7.4f" INFINITY_CHAR" %c, %8.4f" INFINITY_CHAR" %c", latn, latc, lngn, lngc);
    else
      sprintf (temp, "%6.4f" INFINITY_CHAR" %c, %6.4f" INFINITY_CHAR" %c", latn, latc, lngn, lngc);
    text_out = temp;
  }
}
...
void Coordinates::printLat (Dstr &text_out) const {
  if (_isNull)
    ...
    if (latn < 0.0) {
      ...
      latc = 'N';
    char temp[80];
    sprintf (temp, "%3.1f" INFINITY_CHAR" %c", latn, latc);
    text_out = temp;
  }
}
...
void Coordinates::printLng (Dstr &text_out) const {
  if (_isNull)
    ...
    if (lngn < 0.0) {
      ...
      lngc = 'E';
    char temp[80];
    sprintf (temp, "%3.1f" INFINITY_CHAR" %c", lngn, lngc);
    text_out = temp;
  }
}
