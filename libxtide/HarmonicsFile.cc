...
static void buildMetadata (const StationRef &sr,
  ...
  Dstr tmpbuf;
  Dstr filename = sr.harmonicsFileName;
  // The full path looks bad in metadata; the first column of the HTML gets compressed.
  // If it's inside an Apple app bundle, it's not much use to users anyway.
  // Especially if that's an iOS app.
  int lastSeparator = filename.strrchr('/');
  if (lastSeparator > 0) {
    filename /= (lastSeparator + 1);
  }

  metadata.push_back (MetaField ("Name", sr.name));
  metadata.push_back (MetaField ("In file", filename));
  if (rec.legalese)
