...
class Graph {
public:
  enum GraphStyle {normal, clock, icon};
  Graph (unsigned xSize, unsigned ySize, GraphStyle style = normal);
  virtual ~Graph();

  // angle is a kludge to help out the tide clock icon.
  // clockOrganizer gets the events shown on the clock.
  void drawTides (Station *station,
                  Timestamp nominalStartTime,
                  Angle *angle = NULL,
                  TideEventsOrganizer *clockOrganizer = NULL);

   void findNextSunEvent (TideEventsIterator &it,
    ...
    // Added for OSX drag events.

    // Limits for calculating relative dates.
    const static Timestamp beginningOfTime;
    const static Timestamp endOfTime;
    Timestamp offsetTimeByDeltaX(Station *station,
                                    Timestamp startTime,
                                    double *pDeltaX);
