...
char Dstr::operator[] (unsigned long at_index) const {
  if (!theBuffer)
    return '\0';
  if (at_index >= used)
    return '\0';
...
unsigned Dstr::repstr (const char *X, const char *Y) {
  ...
  if (Xlen > length())
    return 0;
  unsigned i, l = length() - (unsigned)Xlen + 1, repcount=0;
  Dstr temp;
  for (i=0; i<l;)
